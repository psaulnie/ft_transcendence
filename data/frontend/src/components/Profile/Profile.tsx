                ...
                <Grid item container>
                  <Grid item>
                    <Typography variant="h6" align="left" sx={{
                      ...
                      fontWeight: 'bold',
                      color: 'black'
                    }}>{userProfile.username.charAt(0).toUpperCase() + userProfile.username.slice(1)}</Typography>
                  </Grid>
                  {user.username === username && (
                    <Grid>
                      <Button onClick={handleModificationClick} sx={{...editButtonStyle}}
                              startIcon={<BorderColorIcon sx={{color: grey[200]}}/>}/>
                    </Grid>
                  )}
                </Grid>
                <Grid item>
                  <Typography variant="h6" align="left" sx={{
                    ...
                    color: 'black',
                    marginBottom: '-0.2em'
                  }}>Rank: {userProfile.rank}</Typography>
                  <Typography variant="h6" align="left" sx={{
                    fontSize: 16,
                    color: 'black',
                    marginBottom: '-0.2em'
                  }}>Wins: {userProfile.wins}</Typography>
                  <Typography variant="h6" align="left"
                              sx={{fontSize: 16, color: 'black'}}>Losses: {userProfile.loses}</Typography>
                </Grid>
              </Grid>
            </Grid>
          </Grid>
              ...
              userProfile.matchHistory.map((match: any, index: any) => (
                ...
                >
                  ...
                  <Grid item container xs={4} alignItems="center" justifyContent="flex-end">
                    ...
                    <Avatar src={`http://${import.meta.env.VITE_IP}:5000/api/avatar/${match.p1}`} alt="User Avatar"
                            sx={{width: '30px', height: '30px', margin: '0 5px'}}/>
                  </Grid>

                  {/* Scores */}
                  <Grid item container xs={2} alignItems="center" justifyContent="center">
                    <Typography sx={{
                      color: match.scoreP1 > match.scoreP2 ? '#1ABAFF' : '#FC7D07',
                      fontSize: 20,
                      margin: '0 10px'
                    }}>{match.scoreP1}</Typography>
                    <Typography sx={{fontSize: 20, color: 'black', margin: '0 5px'}}>:</Typography>
                    <Typography sx={{
                      color: match.scoreP2 > match.scoreP1 ? '#1ABAFF' : '#FC7D07',
                      fontSize: 20,
                      margin: '0 10px'
                    }}>{match.scoreP2}</Typography>
                  </Grid>

                  {/* Player 2 (avatar and username) */}
                  <Grid item container xs={4} alignItems="center" justifyContent="flex-start">
                    <Avatar src={`http://${import.meta.env.VITE_IP}:5000/api/avatar/${match.p2}`} alt="User Avatar"
                            sx={{width: '30px', height: '30px', margin: '0 5px'}}/>
