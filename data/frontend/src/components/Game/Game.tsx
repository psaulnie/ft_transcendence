...
const Canvas: React.FC = () => {
  ...
  const [rectPosition, setRectPosition] = useState<{ x: number; y: number }>({
    x: 20,
    y: 0,
  });
  ...
  useEffect(() => {
    ...
    const animate = () => {
      if (
        rectPosition.x + rectWidth / 2 < 0 ||
        rectPosition.x + rectWidth / 2 > divSize.height
      )
        ctx!.fillStyle = "white";
      ctx?.clearRect(0, 0, canvas!.width, canvas!.height);
      ctx?.fillRect(
        rectPosition.x - rectWidth / 2,
        rectPosition.y - rectHeight / 2,
        rectWidth,
        rectHeight,
      );
