...
export class UsersStatusService {
	private readonly usersStatus: {clientId: string, accessToken: string, username: string, status: userStatus}[];
	constructor(
	  ...
	  async removeUser(clientId: string) {
		this.usersStatus.splice(this.usersStatus.findIndex((user) => user.clientId === clientId), 1);
	  }
